...
public class UserService implements IUserService {
    ...
    public User create(String name) {
        ...
        /*i have to modify this create method according to the test case given for the usercommand */
        User obj=new User(name, 0); 
    ...
    public List<User> getAllUserScoreOrderWise(ScoreOrder scoreOrder){
     ...
     if(scoreOrder==ScoreOrder.ASC){
        Collections.sort(list1,new ASCsort());
        return list1;
     }
     else{
        Collections.sort(list1,new DSCsort());
        return list1;
     }
    ...
    public UserRegistrationDto withdrawContest(String contestId, String userName) throws ContestNotFoundException, UserNotFoundException, InvalidOperationException {
        ...
        Contest  obj=contestRepository.findById(contestId).orElse(null);
        if(obj==null)
        {
            throw new ContestNotFoundException();
        }
 
        User obj2  =userRepository.findByName(userName).orElse(null);
        if(obj2==null)
        {
            throw new UserNotFoundException();
        }
        
        if(obj2.checkIfContestExists(obj)==false)
        {
            throw new InvalidOperationException();
        }
        if(obj.getContestStatus().equals(ContestStatus.IN_PROGRESS)){
            throw new InvalidOperationException("Cannot Attend Contest. Contest for given id:"+contestId+" is in progress!");
        }
        if(obj.getContestStatus().equals(ContestStatus.ENDED)){
            throw new InvalidOperationException("Cannot Attend Contest. Contest for given id:"+contestId+" is ended!");
        }
     
            obj2.deleteContest(obj);
            userRepository.save(obj2);
            return new UserRegistrationDto(obj.getName(), userName, RegisterationStatus.NOT_REGISTERED);

    }
    
}
class ASCsort implements Comparator<User>{
    @Override
    public int compare(User arg0, User arg1) {
      return arg0.getScore()-arg1.getScore();
    }
}
class DSCsort implements Comparator<User>{
    @Override
    public int compare(User arg0, User arg1) {
